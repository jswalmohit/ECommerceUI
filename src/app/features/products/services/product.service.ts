...
export class ProductService {
  ...
  getById(id: string): Observable<Product | null> {
    ...
    const authTokenValue = localStorage.getItem('user_token') ?? correlationIdValue;
    let headers = new HttpHeaders();
    if (correlationIdValue) headers = headers.set('CorrelationId', correlationIdValue);
    if (authTokenValue) headers = headers.set('AuthToken', authTokenValue);

    return this.http.get<Product>(`${environment.apiBaseUrl}/api/products/${id}`, { headers });
  }
